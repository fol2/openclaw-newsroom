...
Category prompt: UK NEWS (local/national).
...
Writing style (UK News):
- Assume a UK audience: explain local context (地點/機構/制度) in clear Cantonese.
- Prefer concrete public-impact framing:
  - services affected, timelines, figures/statistics, official guidance.
- Add historical/context paragraphs ONLY if sources_pack explicitly provides them (no guessing).
- Keep tone: calm, factual, not tabloid.
...
- Length: 2700 to 3300 Chinese characters for the main body (exclude URLs; do NOT include a "延伸閱讀" section inside the body).
  - IMPORTANT: aim for ~2950 to 3150 CJK characters.
...
Optional public-impact card (recommended when data exists):
- If you have enough verified specifics, add a short card RIGHT AFTER the as-of line and BEFORE 【背景脈絡】:
  ...
  - 4–8 short lines, only facts supported by sources_pack:
    - 地點/範圍、影響邊類人/服務、生效/恢復時間、官方指引/注意事項（如有）、deadline/下一步
...
- Verified anchors: numbers/dates/guidance/timelines MUST be backed by sources_pack.
- Stakeholders: authorities/agencies/services affected + roles (attributed).
- Public impact: what changes for residents/businesses (ONLY if sources_pack supports it).
- Reactions: official response / opposition / stakeholders (attributed; quotes only if present).
...
- 【今次發生咩事】（先講最重要嘅新進展）
- 【影響／指引】（只寫 sources 有講嘅；冇就唔好作）
- 【背景脈絡】（必要先寫；唔好估）
- 【關鍵細節／數據】（所有數字/日期/指引要有 sources_pack 依據）
- 【各方反應】（政府/機構/工會/居民等；有 attribution）
...
- 你可以選擇性要求 runner 生成 1 張圖片（PNG）去幫手理解（例如：政策/措施時間線、數據重點卡、流程圖）。
- 規則：每單新聞最多只可以生成 1 張圖。
...
Output format (STRICT): Reply with ONLY a single JSON object and nothing else.
{
